...
def points_camera2lidar(points, tr_velo_to_cam, r0_rect):
    ...
    # 先算从LiDAR坐标系到cam0_rect坐标系的变换矩阵, 再求逆, 得到
    # 从 cam0_rect坐标系 到 LiDAR 坐标系的变换矩阵
    rt_mat = np.linalg.inv(r0_rect @ tr_velo_to_cam)
    xyz = extended_xyz @ rt_mat.T
    return xyz[..., :3]
...
def remove_outside_points(points, r0_rect, tr_velo_to_cam, P2, image_shape):
    ...
    # 得到视锥的近裁截面的4个角点坐标以及远裁截面的4个角点的坐标, 在cam2坐标系下
    # 视锥概念: https://docs.unity3d.com/cn/2019.4/Manual/UnderstandingFrustum.html
    frustum = get_frustum(image_bbox, C)
    frustum -= T
    # 把视锥体从 cam2 坐标系转换到 cam0_rect 坐标系
    frustum = np.linalg.inv(R) @ frustum.T
    # 把cam0_rect坐标系下的视锥体 8 个角点转换到 LiDAR 坐标系下
    frustum = points_camera2lidar(frustum.T[None, ...], tr_velo_to_cam, r0_rect) # (1, 8, 3)
    # 把一个 3D box / 视锥体的 8 个角点，按面分组成 6 个矩形面。
    group_rectangle_vertexs_v = group_rectangle_vertexs(frustum) #  (n, 6, 4, 3)
    # 根据每个面的 4 个顶点，计算这个面的平面方程, a*x + b*y + c*z + d = 0
    frustum_surfaces = group_plane_equation(group_rectangle_vertexs_v)
    # 计算LiDAR每个 3D 点是否在每个 3D box / 视锥体内部
...
def projection_matrix_to_CRT_kitti(proj):
    """
    将 KITTI 的相机投影矩阵分解为 C、R、T。

    对 P2 来说, 可以理解为把 cam0_rect -> image_2 的投影矩阵分解为:
        P2 = C @ [R | T]
    其中:
        C: cam2 的内参矩阵
        R: cam0_rect 坐标系 -> cam2_rect 坐标系的旋转矩阵
        T: cam0_rect 坐标系 -> cam2_rect 坐标系的平移向量

    因为 P[:, :3] = C @ R, 其中 C 是上三角矩阵、R 是正交旋转矩阵。
    普通 QR 分解适合拆解“正交矩阵 @ 上三角矩阵”, 所以这里先对
    C @ R 求逆, 得到 R^-1 @ C^-1, 再用 QR 分解求出 C 和 R。

    Args:
        proj (np.ndarray, shape=[4, 4]): KITTI 相机投影矩阵, 例如 P2。

    Returns:
        tuple[np.ndarray]: 分解后的 C、R、T。
    """
    # C @ [R|T] = [C@R | C@T]
    CR = proj[0:3, 0:3]
    CT = proj[0:3, 3]
    ...
    # 使用qr分解
    # 因为Rinv是个正交矩阵(Rinv.T @ Rinv = I, 正交矩阵表示一种不改变长度和角度的变换)
    # Cinv是个上三角矩阵, 所以可以得到C和R
...
def get_frustum(bbox_image, C, near_clip=0.001, far_clip=100):
    ...
    # 其实就是针孔相机模型的相似三角形的计算
    # 图像2D框的4个角点 -> 对应4条射线
    # 这4条射线在 near_clip 深度处截一下 -> 得到近截面4个角点
    # 这4条射线在 far_clip 深度处截一下 -> 得到远截面4个角点
    fku = C[0, 0]
    fkv = -C[1, 1]
    u0v0 = C[0:2, 2]
    z_points = np.array(
