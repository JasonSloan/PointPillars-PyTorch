...
class PillarLayer(nn.Module):
    ...
    def forward(self, batched_pts):
        ...
        coors_batch = []
        # 因为这个forward函数将一个batch内的非空体素都cat到了一起, 所以需要在coors前面加上batch_id, 以便后续知道每个体素属于哪个样本
...
class PillarEncoder(nn.Module):
    ...
    def forward(self, pillars, coors_batch, npoints_per_pillar):
        ...
        # 1. 计算每个点相对于其所在 pillar 内真实点均值中心的偏移量(单位: m), 这里的均值中心由该 pillar 内真实点的 xyz 坐标求平均得到, 不是 pillar 的几何中心
        offset_pt_center = pillars[:, :, :3] - torch.sum(pillars[:, :, :3], dim=1, keepdim=True) / npoints_per_pillar[:, None, None] # (p1 + p2 + ... + pb, num_points, 3)

        # 2. 计算每个点相对于其所在 pillar 几何中心的 x/y 偏移量(单位: m), pillar 中心由网格坐标 coors_batch 和 voxel_size 反算得到。
        ...
        # 这里的conv是conv1d, 作用相当于MLP, 只对于每个点的特征做线性变换, 不会混合不同点的特征
        features = F.relu(self.bn(self.conv(features)))  # (p1 + p2 + ... + pb, out_channels, num_points)
        pooling_features = torch.max(features, dim=-1)[0] # (p1 + p2 + ... + pb, out_channels)

        # 6. pillar scatter
        # 前面为了方便并行计算, 把一个 batch 内所有帧的非空 pillar 拼在了一起。
        # 这里根据 coors_batch[:, 0] 中保存的 batch_idx, 再把不同帧的 pillar 分开,
        # 分别散射回各自的 BEV 伪特征图。
        batched_canvas = []
        bs = coors_batch[-1, 0] + 1
        for i in range(bs):
            # 取出第 i 帧对应的所有非空 pillar 坐标和 pillar 特征。
            # cur_coors 的每一行格式为 [batch_idx, coor_x, coor_y, coor_z]。
            cur_coors_idx = coors_batch[:, 0] == i
            cur_coors = coors_batch[cur_coors_idx, :]
            cur_features = pooling_features[cur_coors_idx]

            # 创建当前帧的 dense BEV 画布。初始全 0, 表示空 pillar。
            # shape=(x_l, y_l, out_channel), 例如 (432, 496, 64)。
            canvas = torch.zeros((self.x_l, self.y_l, self.out_channel), dtype=torch.float32, device=device)
            # 把每个非空 pillar 的 64维特征写回它在 BEV 网格中的位置:
            # canvas[coor_x, coor_y] = pillar_feature。
            canvas[cur_coors[:, 1], cur_coors[:, 2]] = cur_features
            # Conv2d 需要 channel-first 格式, 所以从 (x, y, C) 转成 (C, y, x)。
...
class PointPillars(nn.Module):
    def __init__(self,
        ...
        # 下面这几个参数控制 PointPillars 如何把一帧 LiDAR 点云切成 pillar。
        #
        # point_cloud_range = [x_min, y_min, z_min, x_max, y_max, z_max]
        # 表示只看 LiDAR 坐标系中的一块空间:
        #   x: 0 ~ 69.12m       只看车辆前方约 70 米
        #   y: -39.68 ~ 39.68m  只看左右约 80 米宽的区域
        #   z: -3 ~ 1m          只看地面附近到车/行人高度附近的点
        #
        # voxel_size = [0.16, 0.16, 4] 表示每个柱子的底面是 0.16m x 0.16m,
        # 高度是 4m。因为 z 范围正好也是 4m, 所以高度方向只切成 1 层,
        # 这就是 PointPillars 中的 pillar: 只在 BEV 平面上切网格, 不把高度切很多层。
        #
        # 用这组默认值计算网格大小:
        #   x 方向: (69.12 - 0) / 0.16 = 432
        #   y 方向: (39.68 - (-39.68)) / 0.16 = 496
        #   z 方向: (1 - (-3)) / 4 = 1
        # 所以一帧点云会被划成 432 x 496 x 1 个候选 pillar, 后续会散射成
        # shape 约为 (batch_size, feature_channel, 496, 432) 的 BEV 伪图像。
        #
        # max_num_points=32 表示每个非空 pillar 最多保留 32 个点; 点太多就截断,
        # 点太少就 padding。max_voxels=(16000, 40000) 表示训练时每帧最多保留
        # 16000 个非空 pillar, 推理时最多保留 40000 个非空 pillar, 用来控制显存和速度。
    ...
    def forward(self, batched_pts, mode='test', batched_gt_bboxes=None, batched_gt_labels=None):
        ...
        """把点云切成pillar, 得到维度[max_voxel, num_points, c]的pillar原始特征, 以及每个pillar的网格坐标和每个pillar内真实点的数量"""
        # batched_pts: list[tensor] -> pillars: (p1 + p2 + ... + pb, num_points, c), 
        #                              coors_batch: (p1 + p2 + ... + pb, 1 + 3), 
        #                              num_points_per_pillar: (p1 + p2 + ... + pb, ), (b: batch size)
        pillars, coors_batch, npoints_per_pillar = self.pillar_layer(batched_pts)

        """对每个 pillar 内的点做 PointNet 风格编码, 再按 pillar 网格坐标 scatter 回 BEV 平面, 得到维度[bs, out_channel, y_l, x_l]的BEV伪图像"""
        # pillars: (p1 + p2 + ... + pb, num_points, c), c = 4
        # coors_batch: (p1 + p2 + ... + pb, 1 + 3)
        # npoints_per_pillar: (p1 + p2 + ... + pb, )
        ...
        """得到 3 个不同下采样尺度的 BEV 特征图"""
        # xs:  [(bs, 64, 248, 216), (bs, 128, 124, 108), (bs, 256, 62, 54)]
        xs = self.backbone(pillar_features)

        """对 3 个尺度的 BEV 特征图做上采样, 再 concat 成一个尺度的 BEV 特征图"""
        # x: (bs, 384, 248, 216)
        x = self.neck(xs)

        """对 BEV 特征图做 3 个卷积, 分别预测每个 anchor 的类别、回归框和朝向"""
        # bbox_cls_pred: (bs, n_anchors*3, 248, 216) 
        # bbox_pred: (bs, n_anchors*7, 248, 216)
        # bbox_dir_cls_pred: (bs, n_anchors*2, 248, 216)
        ...
        # 根据 BEV 特征图尺寸在每个网格位置生成 3D anchors。
        # 当前配置中有 3 类目标: Pedestrian、Cyclist、Car; 每类使用一组典型尺寸:
        #   Pedestrian: [w=0.6, l=0.8, h=1.73]
        #   Cyclist:    [w=0.6, l=1.76, h=1.73]
        #   Car:        [w=1.6, l=3.9, h=1.56]
        # 每类在同一个 BEV 位置放 2 个朝向的 anchor: yaw=0 和 yaw=1.57。
        # 因此每个 BEV 网格位置共有 3 * 2 = 6 个 anchors。
        # anchors 的 shape=(y_l, x_l, 3, 2, 7), 最后一维为 [x, y, z, w, l, h, yaw]。
        anchors = self.anchors_generator.get_multi_anchors(feature_map_size)
        batched_anchors = [anchors for _ in range(batch_size)]

        if mode == 'train':
            # 为每个 anchor 分配训练监督信号。
            # anchor_target 会根据 anchor 与 GT 3D box 的 BEV IoU 判断正负样本:
            #   正样本: 与某个 GT 的 IoU 足够高, 需要学习类别、bbox偏移和方向类别
            #   负样本: 与所有 GT 的 IoU 都很低, 只参与分类背景监督
            #   忽略样本: 介于正负阈值之间, 不参与 loss
            # 返回的 anchor_target_dict 中包含分类标签、分类权重、bbox回归目标、
            # 回归权重、方向分类标签和方向分类权重。
